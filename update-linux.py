...
async def handle_file_operations():
    ...
    try:
        ...
        src_index = os.path.join(cwd, "index-linux.py")
        dst_index = os.path.join(install_dir, "index-linux.py")

        if os.path.exists(src_index):
            os.replace(src_index, dst_index)
            print("index-linux.py replaced successfully.")
        else:
            print("index-linux.py not found in current directory:", src_index)

        src_backup = os.path.join(cwd, "backup.py")
        dst_backup = os.path.join(install_dir, "backup.py")

        if os.path.exists(src_backup):
            os.replace(src_backup, dst_backup)
            print(f"backup.py moved to {dst_backup}")
        else:
            print("backup.py not found in current directory:", src_backup)

        # Ensure scripts are executable
        for p in (dst_index, dst_backup):
            try:
                if os.path.exists(p):
                    st = os.stat(p)
...
async def on_ready():
    ...
    # Download index-linux.py
    txt_url = "https://raw.githubusercontent.com/Plexisity/announcer/main/latest.txt"
    url_index = get_latest_url(txt_url)
    if not url_index:
        print("Could not get the latest index URL. Exiting.")
        await client.close()
        return
    filename_index = "index-linux.py"
    if os.path.exists(filename_index):
        print(f"File {filename_index} already exists. Deleting it...")
        os.remove(filename_index)
    print("Starting download of index-linux.py...")
    await channel.send("Starting download of index-linux.py...")
    await download_file(url_index, filename_index)

    # Download backup.py
    url_backup = "https://raw.githubusercontent.com/Plexisity/announcer/main/backup.py"
    filename_backup = "backup.py"
    if os.path.exists(filename_backup):
        print(f"File {filename_backup} already exists. Deleting it...")
        os.remove(filename_backup)
    print("Starting download of backup.py...")
    await channel.send("Starting download of backup.py...")
